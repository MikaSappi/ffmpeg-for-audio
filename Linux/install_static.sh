...
cd nasm-2.16.03 && \
./autogen.sh && \
./configure --prefix="/usr/local/ffmpeg-static" && \
make -j$CORES && \
sudo make install
...
cd ~/ffmpeg_sources_static/ffmpeg && \
PKG_CONFIG_PATH="/usr/local/ffmpeg-static/lib/pkgconfig:$PKG_CONFIG_PATH" \
./configure \
...
sudo cp /usr/local/ffmpeg-static/bin/ffmpeg /usr/local/bin/ffmpeg
...
echo ""
echo "This is a STATICALLY LINKED build with no external dependencies."
echo ""
...
echo "Static libraries location: /usr/local/ffmpeg-static"
echo ""
